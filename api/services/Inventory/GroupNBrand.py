...
class GroupNbrands(GroupNbrandsModel):

    def current_month_sales(self, product_group_code, brand_code):
        query = """ 
            ...
            WHERE SM.GroupNBrandCode IN 
                ...
                WHERE productGroupCode = ? and  AND brandCode = ? )"""

        params = [product_group_code,brand_code]  # Initialize params with required value

        return db.get_data(query=query, data=params)
    
    def previous_month_sales(self, product_group_code, brand_code):
        query = """ 
            SELECT Sm.*, s.SkuName as SkuName 
            FROM skuSalesPrevMonth SM 
            LEFT JOIN LiveSku S ON S.code = SM.UbDetailsCode  
            WHERE SM.GroupNBrandCode IN 
                (SELECT Code FROM GroupnBrands 
                WHERE productGroupCode = ? AND brandCode = ? )"""
    ...
    @classmethod
    def get_brands_by_productGroupCode(cls,product_group_code,brand_code):
    
        query = """ Select brandCode, BrandName from GroupNbrands Where ProductGroupCode = ? and BrandCode = ? order by salesRank DESC """

        result = db.get_data(query=query,data=[product_group_code,brand_code])
    ...
    def recent_vendors(self, group_code, brand_code=None):
        query = """ 
            ...
            WHERE gb.productGroupCode = ? AND gb.BrandCode = ? 
        """
        
        params = [group_code,brand_code]  # Always filter by group_code

        result = db.get_data(query=query, data=params)  
        return result
    
    @classmethod
    def recent_paid_vendors(self,group_code, brand_code):
        query = """ 
            SELECT B.*, l.skuName 
            FROM StkPaidBills B 
            LEFT JOIN LiveSku l ON l.Code = B.ItemCode 
    ...
    @classmethod
    def get_basic_gp_info(cls,product_gp_code,brand_code):
        query = """select gb.*,p.category p.ProductCategoryCode as categoryCode from GroupNbrands gb join ProductGroups P ON p.Code = gb.ProductGroupCode Where gb.productGroupCode = ? and BrandCode = ? """

        result = db.get_data(query=query,data=[product_gp_code,brand_code])
    ...
    def detail(cls,product_group_code,brand_code=None):
        ...
        result = {
            "info":cls.get_basic_gp_info(product_gp_code=product_group_code,brand_code=brand_code),
            "Skus":cls.sku_list(product_group_code=product_group_code,brand_code=brand_code),
